...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Spinrewriterapi {
		 ...
		 * @param $email_address = CONSTANT = SPIN_EMAIL_ACCOUNT
		 * @param $api_key = CONSTANT = SPIN_API_KEY
		 */
		function __construct() {
			$this->data = array();
			$this->data['email_address'] = SPIN_EMAIL_ACCOUNT;
			$this->data['api_key'] = SPIN_API_KEY;
		 ...
		 * @return bool
		 */
		function setAddHTMLMarkup($add_html_markup) {
			if ($add_html_markup == "true" || $add_html_markup === true || intval($add_html_markup) == 1) {
				$add_html_markup = "true";
			} else {
				$add_html_markup = "false";
			}
			$this->data['add_html_markup'] = $add_html_markup;
			return true;
		}
		 ...
		 * @return bool
		 */
		function setUseHTMLLinebreaks($use_html_linebreaks) {
			if ($use_html_linebreaks == "true" || $use_html_linebreaks === true || intval($use_html_linebreaks) == 1) {
				$use_html_linebreaks = "true";
			} else {
				$use_html_linebreaks = "false";
			}
			$this->data['use_html_linebreaks'] = $use_html_linebreaks;
			return true;
		}
